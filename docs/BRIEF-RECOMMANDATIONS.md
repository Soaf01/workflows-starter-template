...
3. **Lecture prédictive de son propre texte** : tout scripteur relit ce qu'il a *voulu* écrire, pas ce qui est écrit. Il faut casser la prédiction : délai, changement de format visuel, écoute, chasse guidée.
...
3. **Auto-scan (2 min)** — avant toute révélation : « Relis en mode correcteur. Marque tout ce qui te semble suspect. » → mesure et entraîne son **taux d'auto-détection**, la métrique reine.
...
- **TTS** (relecture audio de ses textes) : Web Speech API dans le navigateur pour commencer (gratuit), avec l'avertissement UX que l'audio ne révèle pas la morphologie muette.
