...
export function GiftScreen() {
	...
	return (
		<div className="screen-pad animate-fade-in pt-4">
			...
			<h1 className="mt-1 font-display text-3xl text-ink">{giftCards.title}</h1>
			...
			<div className="relative mt-5 overflow-hidden rounded-app border border-line">
				...
				<div className="absolute inset-0 flex flex-col justify-between p-5">
					<div className="flex items-center justify-between">
						...
						<span className="font-mono text-xs uppercase tracking-widest text-gold">Gift</span>
					</div>
					<div>
			...
			{sent ? (
				<div className="card mt-6 flex flex-col items-center gap-2 p-6 text-center">
					...
					<h2 className="font-display text-xl text-ink">Gift on its way</h2>
					<p className="text-sm text-muted">{money(amount)} to {to || "your recipient"} — demo only.</p>
				...
				<div className="mt-6 space-y-4">
							...
							{giftCards.denominations.map((d) => (
								<button
									...
									className={`rounded-2xl border py-3 text-sm font-semibold transition-colors ${
										amount === d ? "border-transparent text-[#241a10]" : "border-line text-muted"
									}`}
									style={amount === d ? { background: "linear-gradient(180deg,var(--gold-soft),var(--gold))" } : undefined}
								>
									{money(d)}
								</button>
							))}
					...
					<input className="field" placeholder="Recipient email" inputMode="email" value={to} onChange={(e) => setTo(e.target.value)} />
					<textarea className="field resize-none" rows={2} placeholder="Add a message" value={msg} onChange={(e) => setMsg(e.target.value)} />
					<button onClick={() => setSent(true)} className="btn-primary w-full">Send gift · {money(amount)}</button>
