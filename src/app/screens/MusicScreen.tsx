...
export function MusicScreen() {
	...
	return (
		<div className="screen-pad animate-fade-in pt-4">
			<p className="kicker">{config.music.kicker}</p>
			<h1 className="mt-1 font-display text-3xl text-ink">{config.music.title}</h1>
			<p className="mt-2 text-sm text-muted">{config.music.intro}</p>
			...
			<div className="mt-8 flex flex-col items-center">
				...
				>
					...
					<div
						className="absolute left-1/2 top-1/2 flex h-24 w-24 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full font-display text-3xl text-[#241a10]"
						style={{ background: `linear-gradient(180deg, ${current.color}, ${current.color}bb)` }}
					>
				...
				<button
					onClick={() => toggle()}
					className="mt-5 flex h-16 w-16 items-center justify-center rounded-full text-[#241a10] shadow-gold transition-transform active:scale-95"
			...
			{/* Moods */}
			<p className="mt-8 kicker">Moods</p>
					...
					return (
						<button
							...
							className={`flex flex-col items-center gap-1 rounded-2xl border px-2 py-3 text-xs font-semibold transition-colors ${
								on ? "border-transparent text-[#241a10]" : "border-line text-muted"
							}`}
							style={on ? { background: `linear-gradient(180deg, ${r.color}, ${r.color}cc)` } : undefined}
							aria-pressed={on}
						>
			...
			<p className="mt-4 text-center text-[11px] text-muted">
				Ambience is generated live in your browser — no audio files, nothing copyrighted.
			</p>
		</div>
	);
}
