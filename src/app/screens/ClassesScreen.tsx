...
export function ClassesScreen() {
	...
	const close = () => {
		setSelected(null);
		window.setTimeout(() => { setBooked(false); setName(""); setEmail(""); }, 300);
	};

	return (
		<div className="screen-pad animate-fade-in pt-4">
			<p className="kicker">{classes.kicker}</p>
			<h1 className="mt-1 font-display text-3xl font-bold text-ink">{classes.title}</h1>
					...
					<article key={c.id} className="card overflow-hidden">
						...
						<div className="p-4">
							...
							<div className="mt-3 flex flex-wrap gap-2 text-[11px] text-muted">
								...
								<span className="rounded-full border border-line px-2.5 py-1">{t("classes.spots", { n: c.spots })}</span>
							</div>
							<button onClick={() => setSelected(c)} className="btn-primary mt-4 w-full">{t("classes.book")}</button>
						</div>
					</article>
				))}
			</div>

			<Sheet open={!!selected} onClose={close} title={booked ? undefined : t("classes.book")} footer={booked ? null : (
				<button onClick={() => name && email && setBooked(true)} disabled={!name || !email} className="btn-primary w-full">{t("classes.confirm")}</button>
			)}>
				{booked ? (
					<div className="flex flex-col items-center gap-3 py-8 text-center">
						...
						<h2 className="font-display text-xl font-bold text-ink">{t("classes.reserved")}</h2>
						<p className="text-sm text-muted">{selected?.title} · {selected?.date}</p>
						<p className="text-xs text-muted">{t("classes.demoEmail", { email: email || "—" })}</p>
						<button onClick={close} className="btn-ghost mt-2">{t("common.close")}</button>
					</div>
				) : (
					<div className="space-y-3 py-1">
						...
						<input className="field" placeholder={t("common.email")} inputMode="email" value={email} onChange={(e) => setEmail(e.target.value)} />
